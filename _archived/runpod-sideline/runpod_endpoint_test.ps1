...
curl -X POST "https://api.runpod.ai/v2/$EndpointId/run" `
  ...
  -d '{"input":{"mode":"streaming","streaming":true,"roomName":"$RoomName","sessionId":"$SessionId","livekitToken":"$LivekitToken","customLivekitUrl":"$LivekitUrl","sourceImage":"$ImageUrl","ingestionToken":"smoke-test-token"}}'
