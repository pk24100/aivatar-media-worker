...
# Lazy xfuser imports: xfuser initializes UCX/NCCL at import time which
# corrupts GPU memory snapshot state. Only import when distributed is actually
# initialized (multi-GPU). Single-GPU never needs these.
...
def _ensure_xfuser():
    global _get_sequence_parallel_rank, _get_sequence_parallel_world_size, _get_sp_group, _xFuserLongContextAttention
    if _get_sequence_parallel_rank is not None:
        return
    ...
    _get_sequence_parallel_rank = get_sequence_parallel_rank
    _get_sequence_parallel_world_size = get_sequence_parallel_world_size
    _get_sp_group = get_sp_group
    _xFuserLongContextAttention = xFuserLongContextAttention
try:
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6):
        ...
        if self.use_usp:
            _ensure_xfuser()
            self.sp_size = _get_sequence_parallel_world_size()
            self.sp_rank = _get_sequence_parallel_rank()
        else:
            self.sp_size = 1
            self.sp_rank = 0
    ...
    def forward(self, x, freqs, grid_sizes):
        ...
        if self.use_usp:
            ...
            x = _xFuserLongContextAttention(attn_type=attn_type)(
                None,
