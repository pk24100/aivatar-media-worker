...
def get_cond_image_dict(cond_image_path_or_dir, use_face_crop):
    ...
    if os.path.isdir(cond_image_path_or_dir):
        ...
        cond_image_dict = {os.path.splitext(os.path.basename(cond_image))[0]: get_image(cond_image, use_face_crop) for cond_image in cond_image_list}
    else:
...
def get_cached_lite_components(checkpoint_dir, wav2vec_dir, device, param_dtype):
    ...
    if cached_components is not None:
        logger.info("Reusing cached Lite model components")
...
class FlashHeadPipeline:
    def __init__(
        ...
        if self.use_ltx:
            cached_components = get_cached_lite_components(
            ...
            self.vae = cached_components["vae"]
            self.audio_encoder = cached_components["audio_encoder"]
            self.wav2vec_feature_extractor = cached_components["wav2vec_feature_extractor"]
        else:
