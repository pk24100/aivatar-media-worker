...
#
# For fully automated CI builds, set env vars before running:
#   $env:DOCKER_HUB_USER = "pk24100"
#   $env:DOCKER_HUB_TOKEN = "dckr_pat_..."
# When these are present the script auto-logs in, builds, pushes, and cleans up.
# Without them, Docker device-code login is used and the final build step is manual.
#
# Usage: Fill in the configuration values below and run: .\azure_slim_overlay_build.ps1

# ===============================
# CONFIGURATION - Fill these in
# ===============================
$Location             = "australiaeast"
$ResourceGroup        = "aivatar-slim-overlay-rg"
$VmName               = "aivatar-slim-overlay-vm"
$NsgName              = "aivatar-slim-overlay-nsg"
$PublicIpName         = "aivatar-slim-overlay-pip"
$AdminUser            = "azureuser"
...
# The published base image that already contains torch, deps, SoulX-FlashHead source,
# and baked wav2vec2 weights. This is NOT rebuilt.
...
# Optional non-interactive Docker Hub credentials for full automation.
# Set as environment variables before running this script.
$DockerHubUser        = $env:DOCKER_HUB_USER
...
$IsAutomated = -not [string]::IsNullOrWhiteSpace($DockerHubUser) -and -not [string]::IsNullOrWhiteSpace($DockerHubToken)

Write-Host "=== Azure Slim Overlay Build Setup ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "Base image:  $BaseImageTag" -ForegroundColor White
Write-Host "Overlay tag: $OverlayImageTag" -ForegroundColor White
if ($IsAutomated) {
    Write-Host "Mode:        AUTOMATED (DOCKER_HUB_USER + DOCKER_HUB_TOKEN detected)" -ForegroundColor Green
} else {
    Write-Host "Mode:        INTERACTIVE (set DOCKER_HUB_USER + DOCKER_HUB_TOKEN env vars for automation)" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "Step 6-11: Running remote Linux setup and Docker Buildx setup (no model download)..." -ForegroundColor Yellow
$RemoteScript = @"
set -euo pipefail
...
test -f $WorkerDockerfile && echo "$WorkerDockerfile present OK"
...
if ($IsAutomated) {
    Write-Host "Step 12: Automated Docker login, build, and push..." -ForegroundColor Yellow
...
sg docker -c 'docker version'
sg docker -c 'docker buildx use aivatar-builder'
sg docker -c "echo `$DOCKER_HUB_TOKEN | docker login --username `$DOCKER_HUB_USER --password-stdin"
sg docker -c "docker buildx build --platform linux/amd64 --build-arg BASE_IMAGE=$BaseImageTag -f $WorkerDockerfile -t $OverlayImageTag --push ."
"@
    # Pass credentials into the remote shell via environment variables
    $env:DOCKER_HUB_USER = $DockerHubUser
    $env:DOCKER_HUB_TOKEN = $DockerHubToken
    Invoke-RemoteScript -HostName $VmPublicIp -UserName $AdminUser -KeyPath $SshPrivateKeyPath -ScriptContent $BuildScript

    Write-Host ""
    Write-Host "Overlay build and push completed successfully." -ForegroundColor Green
    Write-Host ""

    $cleanupResponse = Read-Host "Delete Azure resource group now? [y/N]"
    if ($cleanupResponse -eq "y" -or $cleanupResponse -eq "Y") {
        ...
        Write-Host "Skipping cleanup. Delete manually when ready:" -ForegroundColor White
...
"@
    ...
    Write-Host "  sg docker -c 'docker buildx build --platform linux/amd64 --build-arg BASE_IMAGE=$BaseImageTag -f $WorkerDockerfile -t $OverlayImageTag --push .'" -ForegroundColor White
    Write-Host ""
    Write-Host "This only uploads the changed app layers; the base layers already exist on Docker Hub." -ForegroundColor White
    Write-Host ""
