...
class VideoPublisher:
    ...
    def __init__(
        ...
        track_name: str = "facemode-video",
        session_id: str = "",
    ):
        self.room = room
        self.fps = fps
        self.max_bitrate = int(max_bitrate or os.getenv("AIVATAR_VIDEO_MAX_BITRATE", "6000000"))
    ...
    def _is_room_disconnected(self) -> bool:
        """Return True if the LiveKit room has gone away or is fully disconnected."""
        if self.room is None:
            return True
        return self.room.connection_state == rtc.ConnectionState.CONN_DISCONNECTED
        ...
        try:
            while True:
                if self._is_room_disconnected():
                    _logger.warning(
                        "VideoPublisher stopping: LiveKit room is disconnected for session=%s",
                        self.session_id,
                    )
                    break
        ...
        try:
            while True:
                ...
                if not _first_frame_logged:
                    _logger.info(
                        "[VP-DIAG] First non-None frame received. Creating track...",
                    )
    ...
    async def _ensure_track(self, frame: np.ndarray):
        ...
        if self._is_room_disconnected():
            _logger.warning(
                "VideoPublisher skipping track creation: LiveKit room is disconnected for session=%s",
                self.session_id,
            )
            return
    ...
    async def _send_frame(self, frame: np.ndarray, frame_source: str = "live"):
        ...
        if self._is_room_disconnected():
            _logger.warning(
                "VideoPublisher dropping frame: LiveKit room is disconnected for session=%s",
                self.session_id,
            )
            return
    ...
    async def _cleanup(self):
        """Unpublish this track without disconnecting a caller-owned room."""
        async with self._cleanup_lock:
            ...
            if participant is not None and track_sid:
                with contextlib.suppress(Exception):
                    await participant.unpublish_track(track_sid)
