# Azure Worker Overlay Build Script
# Builds the THIN worker overlay (Dockerfile.worker) on a small, ephemeral Azure VM.
# It reuses the already-published image as its base, so the ~17.5 GB base layers are
# pulled on Azure's network/disk instead of your laptop. No model download and no
# flash-attn recompile are needed, so the VM can be smaller and shorter-lived than the
# full-build VM in azure_build_vm_setup.ps1.
#
# This script mirrors azure_build_vm_setup.ps1 exactly: it provisions the VM, opens SSH,
# installs Docker + Buildx, and clones the repo. The final `docker login` + build + push
# is run manually over SSH (interactive login), identical in spirit to the full-build flow.
#
# Usage: Fill in the configuration values below and run: .\azure_worker_overlay_build.ps1

# ===============================
# CONFIGURATION - Fill these in
# ===============================
$Location             = "australiaeast"
$ResourceGroup        = "aivatar-overlay-rg"
$VmName               = "aivatar-overlay-vm"
$NsgName              = "aivatar-overlay-nsg"
$PublicIpName         = "aivatar-overlay-pip"
$AdminUser            = "azureuser"
...
Write-Host "=== Azure Worker Overlay Build Setup ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Step 6-12: Running remote Linux setup and Docker Buildx setup (no model download)..." -ForegroundColor Yellow
$RemoteScript = @"
set -euo pipefail
